...
void HttpMgr::postHttpReq(QUrl url, QJsonObject json, ReqId req_id, Modules mod)
{
    ...
    QObject::connect(reply, &QNetworkReply::finished, [reply, this, req_id, mod]() {
        ...
        if (reply->error() != QNetworkReply::NoError) {
            ...
            // 发送信号通知完成
            emit sig_http_finish(req_id, "", ErrorCode::ERR_NETWORK, mod);
            reply->deleteLater();
            return;
        }
        // 无错误则读回请求
        ...
        // 发送信号通知完成
        emit sig_http_finish(req_id, res, SUCCESS, mod);
        reply->deleteLater();
        return;
    });
}

void HttpMgr::slotHttpFinish(ReqId id, QString res, ErrorCode err, Modules mod)
{
